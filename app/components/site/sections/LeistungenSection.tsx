...
import { Code2, Palette, ShoppingCart, Wrench } from "lucide-react";
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
          {[
            ...
            const anchorId = i === 0 ? "leistung-it" : i === 2 ? "leistung-bau" : undefined;
            return (
              <motion.div
                key={i}
                id={anchorId}
                ...
                } ${service.wide ? "border-[#ff1900]/20" : ""} md:hover:-translate-y-1 md:hover:shadow-xl md:hover:shadow-[#ff1900]/10`}
              >
                {service.wide && <div className="absolute top-0 left-8 w-16 h-0.5 bg-gradient-to-r from-[#ff1900] to-transparent rounded-full" />}
                <div className={`flex ${service.wide ? "items-start gap-6 md:gap-8" : "flex-col gap-5"}`}>
                  <div
                    className={`flex-shrink-0 rounded-xl bg-gradient-to-br from-[#ff1900]/15 to-[#ff1900]/5 border border-[#ff1900]/15 flex items-center justify-center transition-all duration-500 group-hover:scale-105 ${
                      service.wide ? "w-14 h-14" : "w-12 h-12"
                    }`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <h3
                      className={`font-bold text-white mb-4 group-hover:text-[#ff1900] transition-colors duration-400 ${
                        service.wide ? "text-xl md:text-2xl" : "text-lg md:text-xl"
                      }`}
                    >
                      {service.title}
                    </h3>
                    <ul className={service.wide ? "grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3" : "space-y-3"}>
